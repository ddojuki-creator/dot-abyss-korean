...
- Ability-detail QA must also check standalone runtime variants for both field 4 and field 5. The detail card can render them in separate `Text` components, so a passing combined key does not cover brace-stripped or green/yellow colorized standalone strings.
