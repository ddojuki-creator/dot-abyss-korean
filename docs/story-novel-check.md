...
16. 대량 번역 직후에는 커밋이나 매니페스트 갱신보다 검수를 먼저 수행한다. 지정 모델의 `review-dialogue-openai.mjs`로 신규 파일의 모든 엔트리를 검토하고 각 blocking 제안을 반영 또는 근거와 함께 기각한 뒤, 각 신규 파일에 `review-novel-layout.mjs --file <path> --fail`을 실행한다. 마지막 파일까지 통과하기 전에는 `update-manifest.mjs`, 로컬 캐시 복사, CDN push를 진행하지 않는다. `--changed` 검사는 커밋 전에만 유효하므로 커밋 후 변경 0개 결과를 통과 증거로 사용하지 않는다.
