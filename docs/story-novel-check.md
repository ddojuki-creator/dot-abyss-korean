...
업데이트마다 최신 전체 캐시에서 발견한 신규/변경 시나리오 ID 목록을 먼저 고정한다. `--cache-since` 결과만 사용하지 않으며, 제목·개요·outgame 번역이 존재한다는 이유로 본문까지 반영됐다고 판단하지 않는다.

각 ID에 대해 아래 네 위치를 exact ID로 대조한다.

1. 전체 캐시 원본 인덱스
2. `translations/novels/<NOVEL_ID>/ko_KR.json`
3. `translations/manifest/ko_KR.json`의 `novels`
4. 게임 로컬 `cache/ko_KR/novels/<NOVEL_ID>.json`

또한 원본 인덱스와 번역 JSON 사이에서 `message`, `dotmessage`, `messageTextCenter`, `l2dmessage`의 종류별 key 수와 전체 key 집합이 같아야 한다. 파일만 존재하거나 `--write-missing-source`가 원문 value로 뼈대를 만든 상태는 완료가 아니다. 문장부호만 있는 연출 key를 제외하고 `value == source`가 1건이라도 있으면 번역 누락으로 처리한다.

2026-07-24 코토노 회귀 검증 ID:

```text
hmn_10110100001 hmn_10110100002 hmn_10110100003
hmr_10110100011 hmr_10110100012 hmr_10110100013
hmr_10110100021 hmr_10110100022 hmr_10110100023
hmr_10110100031 hmr_10110100032 hmr_10110100033
men_10110100001 men_10110100002 men_10110100003
evs_10600010101
```
...
10. `scripts\audit-novel-dialogue-metadata.mjs --all-cached --deep-small-textassets --write-index`로 원본 대사 화자 인덱스를 만들고, `speaker + source` 기준의 호칭/성인 용어 검수를 통과시킨다. 이 감사는 `message`, `dotmessage`, `messageTextCenter`, `l2dmessage`를 모두 보며, 각 speaker가 `names/ko_KR.json`에 번역되어 있는지 자동 검사한다. 쿠레하/마리나/코토노의 서로 다른 `旦那様` 규칙과 `素股/スマタ/すまた=스마타` 규칙도 함께 검사한다. 코토노는 충성 맹세 전 `<user>殿=<user>공`, 맹세 후 `旦那様/旦那=주군`인지 확인한다. `missing-speaker-name` 또는 `untranslated-speaker-name`이 1건이라도 있으면 배포하지 않는다.
...
14. 줄바꿈은 `translation/style-core.md` 기준을 따른다. 현재 스토리 대사 기준은 표시 줄당 약 34자 목표, 36자 초과 시 수정이다. 36자는 대사 전체가 아니라 한 줄 기준이다. 줄이 길면 직역을 고집하지 말고 의미/말투를 유지한 채 자연스럽게 압축하거나 `<br>`을 자연스러운 한국어 구 단위에 넣는다. `조사<br>를`, `느<br>껴서`, `책임감<br>을`처럼 단어/조사/어미가 잘리는 줄바꿈은 실패로 본다. 원문 key는 그대로 두되 번역 value의 줄바꿈은 원문 위치·개수와 독립적으로 배치한다. 최종 한국어는 최대 두 줄(`<br>` 최대 1개) 안에서 의미와 문장 연결을 보존해야 한다.
15. 신규 `men_...` 파일은 짧은 홈/UI 문구로 간주해 표본 검수하지 않는다. 업데이트에서 추가된 `men_...`의 모든 대사를 `hmn_...`, `hmr_...`와 함께 전수 문맥 검수하고, `アタシ=나`, `前線基地=전선기지`처럼 이미 정해진 자연어·용어가 음차나 직역으로 남지 않았는지 검색한다. GPT 검수 사용 시 지정 모델로 전체 엔트리를 검토하되, `ハンバーグ=햄버그 스테이크` 같은 원문 의미와 캐릭터 카드보다 모델 제안을 우선하지 않는다.
16. 대량 번역 직후에는 커밋이나 매니페스트 갱신보다 검수를 먼저 수행한다. `translation/direct-review.md`의 직접 검수 또는 지정 모델의 `review-dialogue-openai.mjs`로 신규 파일의 모든 엔트리를 검토하고 각 blocking 제안을 반영 또는 근거와 함께 기각한 뒤, 각 신규 파일에 `review-novel-layout.mjs --file <path> --fail`을 실행한다. API 크레딧 부족만으로 직접 번역·검수까지 중단하지 않는다. 마지막 파일까지 통과하기 전에는 `update-manifest.mjs`, 로컬 캐시 복사, CDN push를 진행하지 않는다. `--changed` 검사는 커밋 전에만 유효하므로 커밋 후 변경 0개 결과를 통과 증거로 사용하지 않는다.
...
- 변경된 노벨은 고정 2줄 대화창 기준 줄당 목표 34자, 최대 36자이며 `<br>`은 최대 1개다. 35~36자는 `watch`로 검수하되 허용 범위이며, `review-novel-layout.mjs --changed --fail`은 `autoReflow=0 manual=0 blocking=0`으로 통과해야 한다.
