...
For the fixed two-line story dialogue window, use at most one `<br>`. Target at most 34 visible Korean characters per rendered line and never exceed 36 in an approved final layout. Rephrase concisely without losing meaning; if no faithful wording fits, preserve the meaning and leave an unresolved layout finding for review rather than dropping information or adding a third line to claim a pass.
...
Treat all five commands (`message`, `dotmessage`, `messageTextCenter`, `messageTextUnder`, `l2dmessage`) as required source coverage. Include every `messageTextCenter` location/time transition and `messageTextUnder` narration; translate `<size=48>――place</size>`, `翌日`, and `数日後` exact keys across the complete `evs_*` event set.
